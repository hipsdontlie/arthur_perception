...
// Include libraries for cameracatkin
#include <algorithm>
#include <deque>
#include <iomanip>
#include <iostream>
...
// geometry_msgs::TransformStamped probetip_transformStamped;
...
/*
Function to collect pointcloud
*/
// int collectPointCloud(int nunPoints, int freq){
//     if(freq == 0){
//         // Collect 4 points for calibration
...
int publishData(tf2_ros::TransformBroadcaster br, ros::Publisher* pointcloud_pub, ros::Publisher* marker_pose_publisher, ftkMarker* marker
    ...
    // Distance 
    // static float first_point_x = marker_transform.transform.translation.x;
    // static float first_point_y = marker_transform.transform.translation.y;
    // static float first_point_z = marker_transform.transform.translation.z;

    // auto dist = sqrt(pow((marker_transform.transform.translation.x - first_point_x), 2) +
    //             pow((marker_transform.transform.translation.y - first_point_y), 2) +
    //             pow((marker_transform.transform.translation.z - first_point_z), 2));
    ...
    time_elapsed = ros::Time::now().toNSec() - publish_time.toNSec();
    cout << "Time elapsed since publishing: " << time_elapsed << endl; 
    if(points_added <= num_points){

        // try{
        //     cout << "Found transform\n";
        //     probetip_transformStamped = tfBuffer.lookupTransform("probetip", "camera", ros::Time(0), ros::Duration(10));
        //     // ros::Duration(1.0).sleep();
        //     cout << "After..\n";
        //     *iter_x = probetip_transformStamped.transform.translation.x;
        //     *iter_y = probetip_transformStamped.transform.translation.y;
        //     *iter_z = probetip_transformStamped.transform.translation.z;
        ...
        //     publish_time = cloud.header.stamp;
        ...
        if (points_added > num_points){
            cout << "Collected 500 points. Restart to collect new set!" << endl;
        }

        else{
            cout << "Waiting...   " << "Time elapsed since publishing" << time_elapsed << endl;
        }
    }
    return 0;
